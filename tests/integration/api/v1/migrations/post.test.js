...
async function cleanDatabase(){
    ...
    await database.query({text: "drop schema public cascade; create schema public;"});
}
...
test("POST /api/v1/migrations should return status 200", async () => {
    ...
    const migrationCount = await database.query({text: "SELECT COUNT(*)::int FROM pgmigrations"});
